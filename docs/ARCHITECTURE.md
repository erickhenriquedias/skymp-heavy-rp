...
- **Tabelas Principais:** `accounts`, `discord_identities`, `steam_identities`, `characters`, `character_inventory`, `audit_logs`, `whitelist_applications`, `discord_role_access`, `staff_roles`, `factions`, `holds`, `properties`, `market_stalls`, `crafting_recipes`, `crafting_ingredients`. O schema completo está em `skymp/packages/database/schema.sql` mais as migrations `v2`–`v20`. `setup-db.js` e o boot as aplicam em ordem numérica sob lock, registrando nome e checksum em `schema_migrations`; v17 vincula cada candidatura ao personagem exato, v18 registra concessões verificadas pelo bot do Discord, v19 adiciona o lease exato por conexão e v20 guarda Steam ID secundário vindo de conexão verificada pelo Discord.
...
- O escopo `connections` observa uma conta Steam verificada e a registra apenas
  para busca/auditoria; Discord permanece a única autenticação e autoridade.
...
- **`GET /mods.json`**: envelope Ed25519 de paridade v2 (`{files, loadOrder}` no payload), gerado offline por `scripts/generate-mods-manifest.js` a partir da pasta `Data/` de referência. Manifesto ausente, expirado, adulterado, incompatível ou vazio responde **503**.
